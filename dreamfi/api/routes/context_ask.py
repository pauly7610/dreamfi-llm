...
from dreamfi.context.bundle import ContextBundle, persist_bundle
...
def _effective_asker(request: Request, body_asker: str) -> str:
    auth_method = getattr(request.state, "auth_method", None)
    actor_id = getattr(request.state, "actor_id", None)
    if auth_method in {"basic", "bearer"} and actor_id:
        return str(actor_id)
    return body_asker or str(actor_id or "")


def _check_grounding(bundle: ContextBundle) -> None:
    valid_refs = {
        ref
    ...
    for citations in (claim.citation_ids for claim in bundle.claims):
        if not citations:
            raise UngroundedClaimError(
                status_code=412,
                detail="ungrounded_claim_rejected",
            )
        if any(citation not in valid_refs for citation in citations):
            raise UngroundedClaimError(
                status_code=412,
                detail="ungrounded_claim_rejected",
            )
...
def ask(
    ...
    asker = _effective_asker(request, body.asker)
    hint = TopicHint(
