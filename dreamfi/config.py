...
class Settings(BaseSettings):
    ...
    fallback_llm_models: list[str] = Field(
        default_factory=lambda: ["claude-3-5-haiku-20241022", "gpt-5.5"]
    )
    ...
    dreamfi_connector_http_timeout_seconds: float = Field(default=20.0)
    dreamfi_connector_sync_batch_size: int = Field(default=50)
    dreamfi_connector_secret_key: str = Field(default="")
    dreamfi_source_packet_document_limit: int = Field(default=100)
    dreamfi_source_packet_history_per_source: int = Field(default=6)
    dreamfi_demo_source_packets_enabled: bool = Field(default=True)
    dreamfi_source_contradiction_max_count: int = Field(default=8)
    dreamfi_source_refresh_cadence_days: int = Field(default=1)
    dreamfi_source_refresh_schedule_history_limit: int = Field(default=5)
    dreamfi_source_sync_run_history_limit: int = Field(default=75)
    dreamfi_evidence_export_audit_limit: int = Field(default=50)
    ...
    dreamfi_skill_mining_min_traces: int = Field(default=3)
    dreamfi_skill_mining_window_days: int = Field(default=30)
    dreamfi_skill_mining_eval_seed_limit: int = Field(default=5)
