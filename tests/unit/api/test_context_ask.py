...
from dreamfi.api.deps import get_context_builder, get_db_session, get_llm_router
from dreamfi.config import get_settings
from dreamfi.context_connectors.confluence import ConfluenceClient
from dreamfi.context_connectors.jira import JiraClient
...
from dreamfi.db.models import Base, ContextBundleRow, ContextQuestionRow
...
def test_ask_returns_grounded_claims_and_records_memory(session: Session) -> None:
    ...
    bundles = list(session.scalars(select(ContextBundleRow)))
    assert len(bundles) == 1
    assert bundles[0].bundle_id == body["bundle_id"]
...
@respx.mock
def test_ask_rejects_unknown_citation_with_412(session: Session) -> None:
    _mock_jira(["ACT-411"])
    llm = FakeModelRouter(
        responses={
            "ACT-411": ClaimDistillation(
                claims=[
                    ClaimDraft(
                        statement="This claim cites a source that was not fetched",
                        source_refs=["jira:FAKE-999"],
                        confidence=0.6,
                    )
                ]
            )
...
def test_production_context_builder_dependency_has_session(session: Session) -> None:
    app = create_app()
    llm = FakeModelRouter(
        responses={
            "empty context": ClaimDistillation(claims=[], open_questions=[]),
        }
    )
    app.dependency_overrides[get_db_session] = lambda: (yield session)  # type: ignore[misc]
    ...
    resp = client.post(
        ...
        json={
            "workspace_id": "w1",
            "question": "empty context",
        },
    )
    assert resp.status_code == 200, resp.text
    assert session.scalar(select(ContextBundleRow)) is not None


@respx.mock
def test_private_memory_uses_authenticated_actor_not_body_asker(
    session: Session, monkeypatch: pytest.MonkeyPatch
) -> None:
    from dreamfi.context.memory import record_question

    monkeypatch.setenv("DREAMFI_AUTH_ENABLED", "true")
    monkeypatch.setenv("DREAMFI_AUTH_USERNAME", "bob")
    ...
    client = _client_with_builder(session, _make_builder(session, llm))
    credentials = base64.b64encode(b"bob:secret").decode("ascii")
